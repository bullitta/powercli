...
foreach ($vm in $array_vm) {
     ...
     #sostituisci gli spazi
      ...
      # aggiunge il  nome host e l'ambiente  a inizio riga 
         ...
         if ($vm -match "centos6_64Guest"){
                ($vm -replace 'centos6_64Guest',';CentOS 6 Linux (64-bit)')| out-file -FilePath $csvName -Append}   
        ...
        if ($vm -match "win2000AdvServGuest"){
            ($vm -replace 'win2000AdvServGuest',';Microsoft Windows 2000 Server')| out-file -FilePath $csvName -Append}
         
        if ($vm -match "winLonghornGuest"){
            ($vm -replace 'winLonghornGuest',';Microsoft Windows Server 2008 (32-bit)')| out-file -FilePath $csvName -Append}
         ...
         if ($vm -match "winNetEnterpriseGuest"){
            ($vm -replace 'winNetEnterpriseGuest',';Microsoft Windows Server 2003 (32-bit)')| out-file -FilePath $csvName -Append}
         
         if ($vm -match "winNetStandardGuest"){
            ($vm -replace 'winNetStandardGuest',';Microsoft Windows Server 2003 Standard (32-bit)')| out-file -FilePath $csvName -Append}
