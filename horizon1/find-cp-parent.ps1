...
.\find-cp-parent.ps1 -datastore ASERl -cpu 8 -mem 32 -date "12/07/2020"
...
$VM = Get-VM -name cp-parent* -datastore $datastore

if (-not $VM) { Write-Host "nessun cp-parent presente nel datastore, verificare"
                   Exit
                  }
...
# PROVO a selezionare la cp-parent in base al num delle cpu e alla ram delle VDI originali
