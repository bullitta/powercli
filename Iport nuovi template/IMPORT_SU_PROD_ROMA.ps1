<# 
questo script importa le vm nell'ambiente di PRODUZIONE di Roma Europa
prima di lanciarlo occorre collegarsi al server con il comando:
Connect-VIServer -server prm1pcmvcpyl01v.rete.poste -u .... -p .....
...
$DATASTORE = "P_CAMPUS_COMP_LUN005_HITACHI_SHARED"

$Folder = Get-Folder -Name Templates
$PORTGROUP = Get-VDPortgroup -name vxw-dvs-2165-virtualwire-9-sid-5004-EUR-LS-PROD-02
...
if ( $Array_vm[0] -match '^WIN' ) {$VMHOST = Get-VMHost -name prm1win*esx01*}
if ( $Array_vm[0] -match '^RHE' -Or $Array_vm[0] -match '^SLE') {$VMHOST = Get-VMHost -name prm1lin*esx01*}
if ( $Array_vm[1] -match 'SQL' -Or $Array_vm[1] -match 'ORACLE' ) {$VMHOST = Get-VMHost -name prm1db*esx01*}
