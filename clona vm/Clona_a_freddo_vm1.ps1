...
.\CLONA_a_freddo_VM1.PS1 -server Pas87,cdert4e3 -suffix 06052020_patch 
...
#Verifica lo stato della vm, se running avvisa e non esegue il cloning

$VM = Get-vm -name $vm
if ($VM.extensiondata.guest.gueststate -eq "running") {
                                                       write-host "La macchina $vm risulta in stato running"
                                                       Exit
                                                        }
